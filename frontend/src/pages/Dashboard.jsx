...
export default function Dashboard() {
  ...
  const loadAlerts = useCallback(async () => {
    try {
      ...
      // Fallback: try /alerts/recent (current public endpoint)
      try {
        const fallback = await api.recent();
        setAlerts(fallback.data || []);
        setError('');
      } catch {
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-auto p-8">
          ...
          <div className="flex gap-6">
            {/* Alerts table */}
            <div className={`flex-1 bg-white rounded-xl border border-gray-200 overflow-hidden ${selected ? 'lg:max-w-none' : ''}`}>
              {loading ? (
                        ...
                        >
                          <td className="px-5 py-3.5">
                            <p className="text-sm font-medium text-gray-900 truncate max-w-xs">
                              {alert.rule_name || alert.alert_rule_name || 'Alerta sin nombre'}
                            </p>
                            {alert.resource && (
                          ...
                          <td className="px-5 py-3.5 text-sm text-gray-500">
                            {relativeTime(alert.created_at || alert.fired_at)}
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-5">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-900">
                      {selected.rule_name || selected.alert_rule_name || 'Sin nombre'}
                    </p>
                    {selected.resource && (
                      <p className="text-xs text-gray-500 mt-1 break-all">{selected.resource}</p>
                    )}
                  ...
                  {/* Diagnosis */}
                  {(selected.diagnosis || selected.summary) && (
                    <div>
                  ...
                  <div className="text-xs text-gray-400 space-y-1 pt-2 border-t border-gray-100">
                    <p>Recibida: {relativeTime(selected.created_at || selected.fired_at)}</p>
